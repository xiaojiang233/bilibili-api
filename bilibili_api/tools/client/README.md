...
- ▶️ **视频播放** - 内置视频播放器，支持在线播放
- 💬 **弹幕系统** - 完整的弹幕显示功能，实时滚动弹幕
- 📺 **直播浏览** - 浏览热门直播间，一键打开直播
- 📝 **动态展示** - 查看全站动态更新
- 👤 **用户信息** - 查看用户个人资料、粉丝、关注等信息
- 🖼️ **视频封面** - 自动加载视频和直播间封面图片
- 📝 **视频详情** - 查看完整的视频信息、统计数据和简介
- 👤 **用户登录** - 支持用户登录功能（需要手动配置凭据）
- 🌐 **浏览器打开** - 支持在浏览器中打开视频、直播间和动态
- 📋 **链接复制** - 一键复制视频链接到剪贴板
- 📱 **响应式布局** - 自适应窗口大小的界面设计
- 🔧 **模块化设计** - 清晰的代码结构，易于扩展
- ⚡ **异步加载** - 所有网络请求异步处理，界面流畅不卡顿
...
4. 播放器功能：
   ...
   - **音量控制**: 调节播放音量
   - **弹幕开关**: 切换弹幕显示/隐藏
   - **实时弹幕**: 彩色弹幕从右向左滚动，带黑色描边
5. 弹幕会自动根据视频播放时间显示

**注意**: 
- 部分视频可能需要大会员权限
- 如果视频无法播放，可以使用"在浏览器中打开"功能
- 弹幕会在播放时自动加载并显示
...
1. 点击视频详情对话框中的 UP主链接（即将实现）
2. 或在用户信息对话框中输入UID
3. 查看用户头像、签名、粉丝、关注数等信息
4. 可以在浏览器中打开用户主页
...
- **DynamicCard** - 动态卡片组件，显示动态信息
- **DanmakuLabel** - 弹幕标签组件，带描边效果的彩色弹幕
- **VideoPlayerDialog** - 视频播放器对话框，支持视频播放和弹幕显示
- **VideoDetailDialog** - 视频详情对话框，展示完整视频信息
- **UserProfileDialog** - 用户信息对话框，显示用户详细资料
...
- **AllDynamicsWorker** - 全站动态工作线程，获取最新动态
- **UserDynamicsWorker** - 用户动态工作线程，获取指定用户动态
- **VideoPlayUrlWorker** - 视频播放地址工作线程，获取视频播放URL
- **DanmakuWorker** - 弹幕工作线程，获取视频弹幕数据
- **VideoInfoWorker** - 视频信息工作线程，获取视频CID等信息
- **UserInfoWorker** - 用户信息工作线程，获取用户资料
...
✅ 响应式布局  
✅ 异步加载机制  
✅ 完整的Material Design样式  

## 特色功能

### 🎬 视频播放系统 ⭐ 新增！
- **内置播放器**: 使用 PyQt6 Multimedia 实现的完整视频播放器
- **视频播放**: 支持在线播放 Bilibili 视频（需要适当权限）
- **弹幕显示**: 实时滚动弹幕，从右向左滑动
- **弹幕特效**: 彩色弹幕，带黑色描边，确保可读性
- **播放控制**: 
  - 播放/暂停按钮
  - 可拖动的进度条
  - 音量滑块调节
  - 弹幕开关
- **自动同步**: 弹幕根据视频播放进度自动显示
- **异步加载**: 视频地址和弹幕数据异步获取，不阻塞UI

### 完整的视频浏览体验
- 推荐、热门、搜索三种视频发现方式
- 高清封面图片自动加载
- 详细的视频信息展示（播放、点赞、投币、收藏、弹幕、评论）
- 一键播放视频或在浏览器中观看
- 复制链接分享
